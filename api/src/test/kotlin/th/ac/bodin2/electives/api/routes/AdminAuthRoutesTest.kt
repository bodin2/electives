...
class AdminAuthRoutesTest : ApplicationTest() {
    ...
    fun `authenticate success`() = runRouteTest {
        ...
        val response = client.postProto(
            "/admin/auth",
            AuthService.AuthenticateRequest(password = "signed-challenge")
    ...
    fun `authenticate no challenge`() = runRouteTest {
        ...
        client.postProto(
            "/admin/auth",
            AuthService.AuthenticateRequest(password = "some-signature")
        ).assertNotFound()
    }
    ...
    fun `authenticate invalid signature`() = runRouteTest {
        ...
        val response = client.postProto(
            "/admin/auth",
            AuthService.AuthenticateRequest(password = "bad-signature")
        )
